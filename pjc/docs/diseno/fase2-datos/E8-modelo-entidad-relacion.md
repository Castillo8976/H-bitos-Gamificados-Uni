...
El MER Conceptual identifica las **13 entidades** del dominio, clasifica cada una como fuerte, débil o de asociación, y define las **13 relaciones** con sus cardinalidades y justificación de negocio. Toda entidad y relación listada aquí corresponde directamente a una tabla en el Modelo Relacional y a su definición en el DDL.

> **Nota de corrección (agosto 2026):** se eliminó la entidad `reporte` por decisión del equipo. Sus tres campos eran calculables en tiempo real (`COUNT`/`SUM` sobre `tarea`, `sesion_estudio` y `punto`), por lo que mantenerla como tabla propia era una redundancia sin justificación suficiente. El "Tablero de Avance Personal" ahora consulta estos totales directamente.
...
| **nivel_cuenta** | Fuerte | id_nivel | Catálogo global de niveles. El nivel alcanzado se determina comparando los puntos acumulados con puntos_minimos. |
...
| **notificacion** | Débil | id_notificacion + FK id_cuenta | No existe sin cuenta destinataria. ON DELETE CASCADE. |
...
- `sesion_estudio`, `punto`, `reto`, `meta`, `preferencia_visual` y `notificacion` dependen de **cuenta**: si se elimina la cuenta, todos sus datos se eliminan en cascada (ON DELETE CASCADE).
...
- `nivel_cuenta` es una entidad fuerte de catálogo: no depende de una cuenta específica. Una misma definición de nivel puede aplicarse a muchas cuentas y el nivel vigente se obtiene comparando `SUM(punto.cantidad)` con `puntos_minimos`.
...
| **cuenta** | alcanza | nivel_cuenta | **N : 1 (derivada)** | Muchas cuentas pueden alcanzar el mismo nivel. No requiere FK: se determina comparando el total de puntos con puntos_minimos. RF07. |
...
| **cuenta** | recibe | notificacion | **1 : N** | Una cuenta puede recibir muchas notificaciones in-app; cada notificación pertenece a una sola cuenta. RF04, RNF15. |

---

## 2.3 Diagrama Textual del MER

El siguiente diagrama representa las relaciones entre entidades usando notación simplificada. Las líneas dobles (`══`) indican una relación 1:1 y las líneas continuas con flecha (`──→`) representan relaciones 1:N o N:M persistidas en el modelo. La línea punteada (`- - -→`) representa una relación conceptual derivada que no utiliza una FK. Para determinar el nivel vigente, el sistema compara `SUM(punto.cantidad)` con `nivel_cuenta.puntos_minimos`.
...
ENTIDADES FUERTES (independientes):
  ...
  [nivel_cuenta] ←── catálogo global de niveles por umbral de puntos
...
ENTIDADES DÉBILES (dependen de una entidad fuerte):
  ...
  [notificacion]       depende de → cuenta
...
RELACIONES CLAVE:
  ...
  cuenta ── 1:N ──> punto
  cuenta - - N:1 - -> nivel_cuenta  [derivada de SUM(punto.cantidad)]
...
| nivel_cuenta | §13 | Tabla nivel_cuenta | ✓ | CREATE TABLE nivel_cuenta |
| notificacion | §14 | Tabla notificacion | ✓ | CREATE TABLE notificacion |
